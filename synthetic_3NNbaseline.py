...
def test_all_configurations(X, y, dataset_name):
    ...
    # Baseline: KNN with all training data
    ...
    for config in configurations:
        ...
        try:
            ...
            # Train KNN with prototypes
            X_prototypes = X_train[all_prototype_indices]
            y_prototypes = y_train[all_prototype_indices]

            knn_prototypes = KNeighborsClassifier(n_neighbors=min(3, len(X_prototypes)))
...
def visualize_dataset_and_prototypes(result):
    ...
    plt.suptitle(f'{result["dataset_name"]}\n'
    ...
    filename = f'results/{safe_name}_3NN.png'
...
def plot_confusion_matrix(result):
    ...
    ax2.set_title(f'TPS ({format_config_name(best_config["config_name"])})\n'
                 f'G-Mean: {best_config["gmean"]*100:.2f}%')
    ax2.set_xlabel('Predicted')
    ax2.set_ylabel('True')

    plt.suptitle(f'{result["dataset_name"]} - Confusion Matrices',
                fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.show()
...
def create_summary_statistics(all_results):
    ...
    print("\n" + "="*150)
    print("BEST CONFIGURATION FOR EACH DATASET (OPTIMIZED FOR G-MEAN) - bifiltration")
    print("="*150)
    ...
    print("\n" + "="*60)
    print("OVERALL PERFORMANCE STATISTICS (bifiltration)")
    print("="*60)
...
def run_comprehensive_synthetic_study():
    """Run the complete study on all synthetic datasets using bifiltration."""
    print("="*80)
    print("COMPREHENSIVE TDAPS STUDY - bifiltration - EXPANDED")
    print("="*80)
    ...
    if best_df is not None:
        os.makedirs('results', exist_ok=True)
        best_df.to_csv('results/results_synthetic_3NNbaseline.csv', index=False)
        print("\n" + "="*60)
        print("Results saved to: 'results/results_synthetic_3NNbaseline.csv'")
        print("="*60)
