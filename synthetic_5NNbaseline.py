...
def test_all_configurations(X, y, dataset_name):
    ...
    # Baseline: KNN with all training data (5-NN)
    print("\n--- Baseline (All training data, 5-NN) ---")
    ...
    for config in configurations:
        ...
        try:
            ...
            # Train KNN with prototypes (5-NN)
            X_prototypes = X_train[all_prototype_indices]
            y_prototypes = y_train[all_prototype_indices]

            knn_prototypes = KNeighborsClassifier(n_neighbors=min(5, len(X_prototypes)))
...
def visualize_dataset_and_prototypes(result):
    ...
    plt.suptitle(f'{result["dataset_name"]} (5-NN)\n'
    ...
    filename = f'results/{safe_name}_5NN.png'
...
def plot_confusion_matrix(result):
    ...
    ax1.set_title(f'Baseline (All Data, 5-NN)\nG-Mean: {result["baseline_gmean"]*100:.2f}%')
    ...
    ax2.set_title(f'TPS ({format_config_name(best_config["config_name"])}, 5-NN)\n'
                 f'G-Mean: {best_config["gmean"]*100:.2f}%')
    ax2.set_xlabel('Predicted')
    ax2.set_ylabel('True')

    plt.suptitle(f'{result["dataset_name"]} - Confusion Matrices (5-NN)',
                fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.show()
...
def create_summary_statistics(all_results):
    ...
    print("\n" + "="*150)
    print("BEST CONFIGURATION FOR EACH DATASET (OPTIMIZED FOR G-MEAN) - bifiltration  (5-NN)")
    print("="*150)
    ...
    print("\n" + "="*60)
    print("OVERALL PERFORMANCE STATISTICS (bifiltration , 5-NN)")
    print("="*60)
...
def run_comprehensive_synthetic_study():
    """Run the complete study on all synthetic datasets using bifiltration with 5-NN baseline."""
    print("="*80)
    print("COMPREHENSIVE TDAPS STUDY - bifiltration - EXPANDED (5-NN)")
    print("="*80)
    ...
    print("\nBaseline and Comparison: 5-Nearest Neighbor Classifier")
    print("\nExpanded Parameter Grid (H0 only):")
    ...
    if best_df is not None:
        os.makedirs('results', exist_ok=True)
        best_df.to_csv('results/results_synthetic_5NNbaseline.csv', index=False)
        print("\n" + "="*60)
        print("Results saved to: 'results/results_synthetic_5NNbaseline.csv'")
        print("="*60)
...
if __name__ == "__main__":
    ...
    print("Using 5-Nearest Neighbor (5-NN) for baseline and comparison")
