...
def test_all_configurations(X, y, dataset_name):
    ...
    # Baseline: KNN with all training data (1-NN)
    print("\n--- Baseline (All training data, 1-NN) ---")
    ...
    for config in configurations:
        ...
        try:
            ...
            # Train KNN with prototypes (1-NN)
            X_prototypes = X_train[all_prototype_indices]
            y_prototypes = y_train[all_prototype_indices]

            knn_prototypes = KNeighborsClassifier(n_neighbors=min(1, len(X_prototypes)))
...
def visualize_dataset_and_prototypes(result):
    ...
    plt.suptitle(f'{result["dataset_name"]} (1-NN)\n'
    ...
    filename = f'results/{safe_name}_1NN.png'
...
def plot_confusion_matrix(result):
    ...
    ax1.set_title(f'Baseline (All Data, 1-NN)\nG-Mean: {result["baseline_gmean"]*100:.2f}%')
    ...
    ax2.set_title(f'TPS ({format_config_name(best_config["config_name"])}, 1-NN)\n'
                 f'G-Mean: {best_config["gmean"]*100:.2f}%')
    ax2.set_xlabel('Predicted')
    ax2.set_ylabel('True')

    plt.suptitle(f'{result["dataset_name"]} - Confusion Matrices (1-NN)',
                fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.show()
...
def create_summary_statistics(all_results):
    ...
    print("\n" + "="*150)
    print("BEST CONFIGURATION FOR EACH DATASET (OPTIMIZED FOR G-MEAN) - bifiltration (1-NN)")
    print("="*150)
    ...
    print("\n" + "="*60)
    print("OVERALL PERFORMANCE STATISTICS (bifiltration, 1-NN)")
    print("="*60)
...
def run_comprehensive_synthetic_study():
    """Run the complete study on all synthetic datasets using bifiltration with 1-NN baseline."""
    print("="*80)
    print("COMPREHENSIVE TDAPS STUDY - bifiltration - EXPANDED (1-NN)")
    print("="*80)
    ...
    print("\nBaseline and Comparison: 1-Nearest Neighbor Classifier")
    print("\nExpanded Parameter Grid (H0 only):")
    ...
    if best_df is not None:
        os.makedirs('results', exist_ok=True)
        best_df.to_csv('results/results_synthetic_1NNbaseline.csv', index=False)
        print("\n" + "="*60)
        print("Results saved to: 'results/results_synthetic_1NNbaseline.csv'")
        print("="*60)
...
if __name__ == "__main__":
    ...
    print("Using 1-Nearest Neighbor (1-NN) for baseline and comparison")
